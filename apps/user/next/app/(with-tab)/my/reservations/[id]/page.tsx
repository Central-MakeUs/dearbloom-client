...
const Header = () => (
  <header className="sticky top-0 z-10 flex h-[52px] items-center bg-neutral-100 px-2">
    <a href="/app/my/reservations" aria-label="뒤로가기" className="flex h-11 w-11 items-center justify-center text-neutral-950">
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
        <path d="m15 18-6-6 6-6" />
      </svg>
    </a>
    <h1 className="absolute left-1/2 -translate-x-1/2 text-head-3 text-neutral-950">문의 상세</h1>
  </header>
);
...
export default async function ReservationDetailPage({
  ...
  const canCancel = !status || (!status.includes('CANCEL') && status !== 'DONE');
      ...
      <div className="px-4 py-3">
        <div className="overflow-hidden rounded-lg bg-neutral-0">
          ...
          <div className="p-4">
            ...
            {d.artistNickname && <p className="mt-0.5 text-caption-1 text-neutral-600">{d.artistNickname}</p>}
