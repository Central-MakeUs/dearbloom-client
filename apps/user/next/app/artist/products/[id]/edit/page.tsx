...
export default async function EditArtworkPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const header = (
    <header className="flex h-[52px] items-center gap-2 border-b border-neutral-200 bg-neutral-0 px-2">
      ...
      <h1 className="text-head-2 text-neutral-950">작품 수정</h1>
    </header>
  );
  ...
  if (!art) {
    return (
      <div className="mx-auto max-w-md">
        {header}
        <p className="px-4 py-16 text-center text-body-4 text-neutral-500">작품을 불러오지 못했어요.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-md">
      {header}
      <EditForm id={art.artworkId} title={art.title} price={art.packageList?.[0]?.price ?? 0} />
    </div>
  );
}
