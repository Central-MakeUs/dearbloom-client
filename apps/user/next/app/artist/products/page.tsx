...
export default async function ArtistProductsPage() {
  ...
  const header = (
    <header className="flex items-center justify-between px-4 pt-6">
      <h1 className="text-head-1 text-neutral-950">내 작품</h1>
  ...
  const body = needLogin ? (
    <div className="flex flex-col items-center gap-3 px-6 py-16 text-center">
      <p className="text-body-4 text-neutral-500">작가 계정으로 로그인해주세요.</p>
      <a href="/app/dev/login" className="rounded-md bg-primary px-5 py-2.5 text-body-4 text-neutral-0">로그인</a>
    </div>
  ) : items.length === 0 ? (
    <div className="flex flex-col items-center gap-3 px-6 py-16 text-center">
      <p className="text-body-4 text-neutral-500">아직 등록한 작품이 없어요.</p>
      <a href="/app/artist/products/new" className="rounded-md bg-primary px-5 py-2.5 text-body-4 text-neutral-0">첫 작품 등록하기</a>
    </div>
  ) : (
