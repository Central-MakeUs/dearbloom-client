...
/**
 * 방 토픽(STOMP)을 구독해 새 메시지와 상대의 읽음을 실시간으로 받는다.
 *
...
export function useLiveMessages(
  ...
  useEffect(() => {
    ...
    client.onConnect = () => {
      ...
      if (!onRead) return;
      client.subscribe(chatRoomReadTopic(roomId), (frame) => {
        try {
          onRead(JSON.parse(frame.body) as ChatReadEvent);
        } catch {
          // 파싱 불가한 프레임은 무시
        }
      });
